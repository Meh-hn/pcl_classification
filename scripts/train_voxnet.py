...
from classification.models.voxnet import VoxNet as voxnet
...
def voxnet_step(model, batch, device, config):
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Train VoxNet model on ModelNet dataset')
    parser.add_argument(
        '--config', 
        type=str, 
        required=True,
    ...
    model = voxnet(num_classes=config['model']['num_classes'], input_size=32)
    model.to(device)
    ...
    train_engine(
        ...
        device=device,
        train_step_fn=voxnet_step, # The function specific to PointNet
        val_step_fn=voxnet_step
    )
